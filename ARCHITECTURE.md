...
Сервис `designer` в `compose.yaml` пока не описан (задача T-24 в работе): на схеме он есть
как часть архитектуры, но контейнером в текущем `docker compose up` не поднимается.
...
4. **Описание по картинке.** Геометрия говорит, из чего слайд собран, но не для какого он
   содержания. Когда уверенность классификации по геометрии ниже 0,7, `parse/describe.py`
   отдаёт модели картинку слайда и то, что уже известно (число слотов, блоков, областей);
   модель возвращает тип, назначение слайда одной фразой и флаг «держится на фото».
   Уверенный разбор модель не трогает.
