...
Экраны в браузере на `http://localhost:8088`: `#/deck` колода по брифу (шаблон, бриф, три варианта, находки аудита, починка, выгрузка pptx/html/pdf), `#/template/<id>` разбор шаблона (палитра, шрифты, шкала кеглей, слайды-образцы с назначением), `#/stage` сцена живого режима.
...
В `examples/` девять колод по одному брифу (`examples/brief.txt`): три выданных шаблона на три варианта вёрстки. В каждом каталоге `examples/<шаблон>/<вариант>/` лист слайдов `sheet.jpg`, `deck.pdf` и карточка прогона `run.json`; `deck.pptx` и `deck.html` в git не лежат, их собирает заново `python examples/build.py` при поднятом `docker compose`. Замер 2026-09-22 на Qwen3.8-27B в LM Studio (RTX 5090): импорт шаблона 77–120 с, три варианта колоды на 12–15 слайдов 201 с с контекстным аудитом первого варианта (`examples/build.log`).

Что на листах видно и не закрыто: на VK WorkSpace паттерн крупного числа даёт мелкую плашку, а не крупную цифру; на VK Education таблица набрана кеглем шкалы шаблона и не влезает в рамку, заголовок титула ложится на фигуры оформления. Аудит эти места находит (`layout.text_overflow`, `layout.overlap`), починка с экрана уменьшает кегль.
