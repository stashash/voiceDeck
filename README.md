...
Обычные паузы дают более ранние финалы; партиалы обновляются примерно каждые 0.5 с аудио плюс время инференса. Это не гарантия задержки на любом компьютере. Времена отдельных предложений интерполируются внутри фразы. Где кончается мысль, решает модель: когда набралось 20 слов, она смотрит на каждое следующее предложение и отвечает, начинается ли с него новая тема (навык `designer/skills/speech-boundary`, около 0,8 с на Qwen3.8 27B). Пауза фрагмент не закрывает, закрывает только молчание дольше 4 с. Пока designer не отвечает, Java-сервис закрывает фрагменты по паузам, как без модели.
